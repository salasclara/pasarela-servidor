/**
 * ThinkingEngine.js
 * PASARELA AI ENGINE™ — TASK-TH-001
 *
 * Paso previo a Anthropic dentro de /api/materialize.
 * Toma la idea original y produce un Editorial Brief estructurado.
 *
 * Flujo:
 *   idea → understandIdea() → classifyIdea() → Editorial Brief → Anthropic
 */

// ── VOCABULARIOS EDITORIALES ──────────────────────────────────────────────────
...
const EDITORIAL_STYLE_MAP = {
  MODA:       'high fashion editorial — Vogue, minimal luxury',
  BELLEZA:    'beauty editorial — íntimo, luminoso, sensorial',
  TALENTO:    'portrait editorial — poderoso, auténtico, aspiracional',
  EVENTOS:    'event editorial — dinámico, celebratorio, social',
  LIFESTYLE:  'lifestyle editorial — cálido, cotidiano, sofisticado',
  EXCLUSIVAS: 'cover story — dramático, exclusivo, cinematográfico',
};

const VISUAL_DIRECTION_MAP = {
  MODA:       'fondo neutro o arquitectónico, iluminación directa, colores saturados o monocromáticos',
  BELLEZA:    'primer plano, piel iluminada, fondo oscuro con bokeh suave',
  TALENTO:    'retrato ambiental, mirada directa a cámara, luz natural o dramática',
  EVENTOS:    'espacio amplio, ambiente festivo, detalles del lugar y las personas',
  LIFESTYLE:  'escena cotidiana estilizada, paleta cálida, composición relajada',
  EXCLUSIVAS: 'composición cinematográfica, alto contraste, atmósfera misteriosa',
};

const AUDIENCE_SIGNALS = {
  profesional:  ['academia', 'agencia', 'casting', 'carrera', 'modelo', 'industria', 'negocio'],
  aspiracional: ['lujo', 'exclusiva', 'gala', 'alfombra', 'premio', 'desfile'],
  comunidad:    ['dallas', 'carrollton', 'texas', 'latina', 'hispana', 'comunidad', 'local'],
  general:      [],
};

const INTENT_SIGNALS = {
  anuncio:     ['inauguramos', 'abrimos', 'lanzamos', 'presentamos', 'anunciamos', 'debut', 'apertura', 'nuevo', 'nueva'],
  celebración: ['celebramos', 'cumpleaños', 'aniversario', 'ganamos', 'logramos', 'éxito'],
  inspiración: ['tips', 'cómo', 'aprende', 'guía', 'secreto', 'transforma', 'mejora'],
  cobertura:   ['estuvo', 'fue', 'asistió', 'participó', 'desfiló', 'se presentó'],
};
...
class ThinkingEngine {
  ...
  /**
   * classifyIdea()
   * Clasifica la idea en categoría, emoción, estilo editorial y dirección visual.
   */
  classifyIdea(idea) {
    const text = idea.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  /**
   * analyze()
   * Método principal — combina understandIdea + classifyIdea.
   * Devuelve el Editorial Brief completo.
   */
  analyze(idea) {
