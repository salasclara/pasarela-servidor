...
fetch('/test-fancy-analytics-history?days=30&limit=50',{cache:'no-store'}).then(r=>r.json()).then(d=>{if(!d.ok)throw Error();history.innerHTML=d.items.length?d.items.map(x=>'<tr><td>'+dateFmt(x.observed_at)+'</td><td>'+esc(x.family||x.tipo_editorial)+'</td><td>'+esc(x.category)+'</td><td class="headline">'+esc(x.headline)+'</td><td>'+esc(x.visual_strategy)+'</td><td>'+esc(x.channel)+'</td><td class="status">'+esc(x.publication_status)+'</td></tr>').join(''):'<tr><td colspan="7" class="muted">Sin publicaciones todavía</td></tr>'}).catch(()=>{history.innerHTML='<tr><td colspan="7" class="muted">Historial no disponible</td></tr>'});
</script></body></html>`;
