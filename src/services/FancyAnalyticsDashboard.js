...
.badge{background:#fff0f6;color:#c6005d;padding:4px 9px;border-radius:20px;font-weight:700}.history{margin-top:14px}.tablewrap{overflow-x:auto}table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;color:#777;font-weight:600;padding:10px 8px;border-bottom:1px solid #eee;white-space:nowrap}td{padding:11px 8px;border-bottom:1px solid #f0edf0;vertical-align:top}.headline{min-width:220px;font-weight:600}.muted{color:#888}.status{color:#17834b;font-weight:700}.safe{margin-top:14px;font-size:12px;color:#777}
...
<section class="panel history"><h2>Historial de publicaciones · últimos 30 días</h2><div class="tablewrap"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Categoría</th><th>Titular</th><th>Estrategia</th><th>Canal</th><th>Estado</th></tr></thead><tbody id="history"><tr><td colspan="7" class="muted">Cargando historial…</td></tr></tbody></table></div></section>
...
function dateFmt(v){if(!v)return '—';const d=new Date(v);return isNaN(d)?'—':d.toLocaleString('es-US',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}
const historyBody=document.getElementById('history');\nfetch('/test-fancy-analytics-history?days=30&limit=50',{cache:'no-store'}).then(r=>r.json()).then(d=>{if(!d.ok)throw Error();historyBody.innerHTML=d.items.length?d.items.map(x=>'<tr><td>'+dateFmt(x.observed_at)+'</td><td>'+esc(x.family||x.tipo_editorial)+'</td><td>'+esc(x.category)+'</td><td class="headline">'+esc(x.headline)+'</td><td>'+esc(x.visual_strategy)+'</td><td>'+esc(x.channel)+'</td><td class="status">'+esc(x.publication_status)+'</td></tr>').join(''):'<tr><td colspan="7" class="muted">Sin publicaciones todavía</td></tr>'}).catch(()=>{historyBody.innerHTML='<tr><td colspan="7" class="muted">Historial no disponible</td></tr>'});
</script></body></html>`;
